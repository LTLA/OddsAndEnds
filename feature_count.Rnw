...
The number of reads (or read pairs) mapped to each exon is counted for the current genome. 
<<results=tex>>=
if (get.exons) { 
	cat("For exonic results, different counts for splice variants are not calculated due to the difficulties of isoform deconvolution.\n")
} else {
