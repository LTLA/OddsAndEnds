...
cmds <- c(cmds, 
sprintf('write.table(output$counts, file="%sic_counts.tsv", sep="\\t", quote=FALSE, row.names=TRUE, col.names=NA)', ifelse(exonic, "exon", "gen")))
...
\n")
   	cmds <- c("require(csaw)",
	    	"sizes <- diags <- list()",
      	 	"for (bam in allBam) {",
	   		"    current <- getPETSizes(bam, minq=minq)", 
