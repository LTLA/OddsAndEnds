...
Finally, we decide whether we want to report the results in terms of exons or in terms of genes.
Gene-level counts are useful for simple DE analyses whereas exon-level counts are useful for 
looking at some differential isoform usage.

<<>>=
get.exons
@

The number of reads (or read pairs) mapped to each exon is counted. If results are
desired in terms of genes, exon-level counts are aggregated to obtain a count for each gene. 
For exonic results, different counts for  splice variants are not calculated due to the difficulties
of isoform deconvolution. 
A copy of the count table is saved for downstream use in the differential expression analysis.

<<results=hide>>=
output <- featureCounts(allBam, annot.inbuilt=genome, isPairedEnd=pet, 
    minMQS=minq, useMetaFeatures=!get.exons)$counts
colnames(output) <- sub("\\.bam$", "", basename(allBam))
write.table(output, file="genic_counts.tsv", sep="\t", quote=FALSE, row.names=TRUE, col.names=NA)
@
