...
This threshold is set at 20 which assumes that 1\% of reads are incorrectly mapped due to ambiguity or other reasons. 
...
\n")
   	cmds <- c("require(csaw)",
	    	"sizes <- diags <- list()",
      	 	"for (bam in allBam) {",
	   		"    current <- getPETSizes(bam, param=readParam(pet='both', minq=minq))", 
