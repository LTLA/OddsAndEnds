...
<<echo=FALSE,results=tex,strip.white=false>>=
cmds <- c(sprintf('output <- featureCounts(allBam, annot.inbuilt=genome, isPairedEnd=pet, minMQS=minq, useMetaFeatures=%s)', as.character(!exonic)),
...
if (pet) {
	cat("
For PET data, we perform additional quality control using the \\texttt{getPETSizes} function in the \\textsf{csaw} package.
...
\n")
   	cmds <- c("require(csaw)",
	    	"sizes <- diags <- list()",
      	 	"for (bam in allBam) {",
	   		"    current <- getPETSizes(bam, param=readParam(minq=minq))", 
...
")
	...
	cmds <- c("by.tech <- split(1:ncol(output$counts), technical)",
			  ...
			  "for (x in 1:length(by.tech)) { new.mat[,x] <- rowSums(output$counts[,by.tech[[x]]]) }",
			  "colnames(new.mat) <- names(by.tech)",
			  "output$counts <- new.mat")
	cat(c("\\begin{Schunk}\n\\begin{Sinput}",
		cat(paste(ifelse(grepl("^\\s", cmds), "+", ">"), cmds), "> head(output$counts)", sep="\n")
		"\\end{Sinput}",
		"\\begin{Soutput}"), sep="\n")
	print(head(output$counts))
	cat("\\end{Soutput}\n\\end{Schunk}\n")
}
@
...
<<>>=
cmds <- sprintf('write.table(output$counts, file="%sic_counts.tsv", sep="\\t", quote=FALSE, row.names=TRUE, col.names=NA)', ifelse(exonic, "exon", "gen"))
cat(c("\\begin{Schunk}\n\\begin{Sinput}",
	paste(">", cmds), 
	"\\end{Sinput}\n\\end{Schunk}"), sep="\n")
eval(parse(text=cmds))
